...
module Gitlab
  class CLI
    ...
    module Helpers
      ...
      def client
        @client ||= Gitlab::Client.new(endpoint: (Gitlab::Client.endpoint || ''))
      end
      ...
      def valid_command?(cmd)
        ...
        Gitlab::Client.actions.include?(command)
      end
      ...
      def record_table(data, cmd, args)
        ...
        table do |t|
          t.title = "Gitlab::Client.#{cmd} #{args.join(', ')}"
          t.headings = keys
      ...
      def record_hash(data, cmd, args, single_value: false)
        ...
        {
          cmd: "Gitlab::Client.#{cmd} #{args.join(', ')}".strip,
          result: result
        }
      end
